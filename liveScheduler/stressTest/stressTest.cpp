...
void usage(char const* progName) {
  cout << "Usage: " << progName << " <rtsp-url> <N>\n";
  cout << "\t(where each <rtsp-url-i> is a \"rtsp://\" URL)\n";
}
TaskScheduler* scheduler;
UsageEnvironment* env;
char eventLoopWatchVariable = 0;

void proc_streeTest(void *arg)
{
  char *progName="stressTest";
  char *rtspURL=(char*)arg;
  srand((int)time(NULL));
  for(;;){
...
/*
static void handlerSIG(int sig){
    if(sig==SIGINT){
        cout<<"SIGINT"<<endl;
    }
...
int main(int argc, char** argv) {
  ...
  /*void (*ohandler) (int);
  ohandler=signal(SIGINT,handlerSIG);
  if(ohandler!=SIG_DFL)
  {
