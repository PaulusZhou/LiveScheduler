...
void MetricsSet::assignMetrics(uint32_t addr,struct loadinfo info){
    ...
    if(metrics==NULL){
        ...
        metrics->agentport=info.agentport;
    }
    else{
...
void MetricsSet1::regulationData(){
    for(Regulation::iterator it=r.begin();it!=r.end();it++){
        it->second.cpu_used=(float)fCpuUsedMin/(float)it->second.cpu_used;
        it->second.concurrecy=(float)fConcurrecyMin/(float)it->second.concurrecy;
        it->second.net_rates=fNetRateMin/it->second.net_rates;
        it->second.responsetime=fResponsetimeMin/it->second.responsetime;
        if(it==r.begin()){
