...
void usage() {
  *env << "Usage: " << progName
       ...
       << " [-t|-T <http-port>]"
       << " [-u <username> <password>]"
       << " -i <schedule-server-ip>"
       << " [-p <schedule-server-port> default 5000]"
...
int main(int argc, char** argv) {
  ...
  *env << "LIVE555 Proxy Server\n"
  ...
  progName = argv[0];
  struct thr_parameter param;
  param.scheduleServerPort=5000;
  if (argc < 3) usage();
  while (argc > 1) {
    ...
    switch (opt[1]) {
    case 'i':{
        param.scheduleServerAddr=argv[2]; 
        ++argv; --argc;
        break;
    }
    case 'p':{
        param.scheduleServerPort=atoi(argv[2]); 
	    ++argv; --argc;
        break;
    }
  ...
  param.proxyServerPort=(int)rtspServerPortNum;
  if (rtspServer == NULL) {
  ...
  char streamNames[1024]={0};
  int i;
  for (i = 1; i < argc; ++i) {
    char const* proxiedStreamURL = argv[i];
    char streamName[128];
    paraseUrl(argv[i],streamName);
    if(i==1){
        strcat(streamNames,streamName);
    }
    else{
        strcat(streamNames," ");
        strcat(streamNames,streamName);
  ...
  param.sources=streamNames;
  pthread_create(&tid,NULL,thr_agent,(void*)&param);
