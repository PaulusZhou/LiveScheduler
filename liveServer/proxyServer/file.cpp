...
slurpfile (char * filename, char **buffer, int buflen)
{
   ...
   if (fd < 0)
      {
         printf("slurpfile() open() error on file %s", filename);
         return SLURP_FAILURE;
      }
   if (*buffer == NULL)
      {
         db = (char*)malloc(buflen);
         dynamic = buflen;
         *buffer = db;
      }
   else
   ...
   if (read_len < 0)
      {
         if (errno == EINTR)
            goto read;
         printf("slurpfile() read() error on file %s", filename);
         close(fd);
         return SLURP_FAILURE;
      }
   else
   ...
   if (read_len == buflen)
      {
         if (dynamic) {
            dynamic += buflen;
            db = (char*)realloc(*buffer, dynamic);
            *buffer = db;
...
update_file (timely_file *tf)
{
    ...
    if(timediff(&now,&tf->last_read) > tf->thresh) {
        ...
        if(rval == SLURP_FAILURE) {
            printf("update_file() got an error from slurpfile() reading %s",
                    tf->name);
        } else {
            tf->last_read = now;
            if (tf->buffer == NULL) {
