...
static net_dev_stats *hash_lookup(char *devname, size_t nlen)
{
  ...
  if ( stats == NULL )
  {
    printf("unable to allocate memory for /proc/net/dev/stats in hash_lookup(%s,%zd)", name, nlen);
    free(name);
    return NULL;
  }
   ...
   if ((proc_net_dev.last_read.tv_sec != stamp.tv_sec) &&
       (proc_net_dev.last_read.tv_usec != stamp.tv_usec))
      {
              ...
              if (p && strncmp (src, "lo", 2) &&
                  strncmp (src, "bond", 4))
                 {
                    ...
                    if ( rbi >= ns->rbi ) {
                       ...
                       printf("update_ifdata(%s) - Overflow in rbi: %lu -> %lu",caller,ns->rbi,rbi);
                    ...
                    if ( rpi >= ns->rpi ) {
                       ...
                       printf("updata_ifdata(%s) - Overflow in rpi: %lu -> %lu",caller,ns->rpi,rpi);
                    ...
                    if ( rbo >= ns->rbo ) {
                       ...
                       printf("update_ifdata(%s) - Overflow in rbo: %lu -> %lu",caller,ns->rbo,rbo);
                    ...
                    if ( rpo >= ns->rpo ) {
                       ...
                       printf("update_ifdata(%s) - Overflow in rpo: %lu -> %lu",caller,ns->rpo,rpo);
        ...
        if ( t <  proc_net_dev.thresh) {
           printf("update_ifdata(%s) - Dubious delta-t: %f", caller, t);
           return;
        }
        ...
        if ((l_bin > 1.0e13) || (l_bout > 1.0e13) ||
           ...
           printf("update_ifdata(%s): %g %g %g %g / %g", caller,
...
metric_init(void)
{
   ...
   if ( rval.int32 == SLURP_FAILURE )
      {
         printf("metric_init() got an error from slurpfile() /proc/cpuinfo");
   ...
   if ( rval.int32 == SLURP_FAILURE )
      {
         printf("metric_init() got an error from slurpfile()");
         rval.int32 = SYNAPSE_FAILURE;
         return rval;
      }
   ...
   if ( dummy == NULL )
      {
         printf("metric_init() got an error from update_file()");
         rval.int32 = SYNAPSE_FAILURE;
         return rval;
      }
...
float find_disk_space(double *total_size, double *total_free)
{
   ...
   while ( fgets(procline, sizeof(procline), mounts) ) {
     ...
     *mode++ = '\0';
     ...
     if (other != NULL) *other = '\0';
      ...
      printf("Counting device %s (%.2f %%)", device, thispct);
      if (!max || max<thispct)
         max = thispct;
   }
   fclose(mounts);
   ...
   *total_free = *total_free / reported_units;
   printf("For all disks: %.3f GB total, %.3f GB free for users.", *total_size, *total_free);
